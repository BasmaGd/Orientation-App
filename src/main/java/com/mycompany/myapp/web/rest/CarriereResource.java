...
public class CarriereResource {
    ...
    public ResponseEntity<Carriere> partialUpdateCarriere(
        ...
        Optional<Carriere> result = carriereRepository
            ...
            .map(existingCarriere -> {
                if (carriere.getNomCarriere() != null) {
                    existingCarriere.setNomCarriere(carriere.getNomCarriere());
                }
                if (carriere.getDescription() != null) {
                    existingCarriere.setDescription(carriere.getDescription());
                }
     ...
     */
    @GetMapping("")
    public List<Carriere> getAllCarrieres(@RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload) {
        log.debug("REST request to get all Carrieres");
        if (eagerload) {
            return carriereRepository.findAllWithEagerRelationships();
        } else {
    ...
    public ResponseEntity<Carriere> getCarriere(@PathVariable("id") Long id) {
        ...
        Optional<Carriere> carriere = carriereRepository.findOneWithEagerRelationships(id);
